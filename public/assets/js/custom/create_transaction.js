...
function addRow(id, code, hours){
  let ids  = document.querySelectorAll('table#addTrans tbody tr td:first-child');
  ...
  c1.innerHTML = `<input type="hidden" value="${id}" name="transferable_id[]"> ${id}`;
  c2.innerText = code;
  c3.innerText = hours;
...
function addRowSubject(id, code, hours){
  let ids  = document.querySelectorAll('table#addSubjects tbody tr td:first-child');
  for (let i = 0; i < ids.length; i++) {
    const element = ids[i];
    if(element.innerText == id){
      return false;
  ...
  let table = document.getElementById('addSubjects');
  let row = table.insertRow();
  row.id = id;
  let c1 = row.insertCell(0);
  let c2 = row.insertCell(1);
  let c3 = row.insertCell(2);
  let c4 = row.insertCell(3);
  let totalHours = document.getElementById('totalHoursSubjects');
  document.getElementById('emptySubjects').style.display = "none";
  c1.innerHTML = `<input type="hidden" value="${id}" name="subject_id[]"> ${id}`;
  c2.innerText = code;
  c3.innerText = hours;
  c4.innerHTML = `<button class="btn btn-danger py-0" id="btnDeleteSubject" data-id=${id} data-hours=${hours}><i class="fa-solid fa-trash-can" id="iDeleteSubject" data-id=${id} data-hours=${hours}></i> Delete</button>`;
  totalHours.innerText = parseInt(totalHours.innerText) + parseInt(hours);
}
...
document.addEventListener("click", function (e){
  ...
  if(e.target.id == 'btnAddSubject' || e.target.id == 'iAddSubject'){
    var id     = e.target.getAttribute('data-id');
    var code   = e.target.getAttribute('data-code');
    var hours  = e.target.getAttribute('data-hours');
    addRowSubject(id, code, hours);
  }
  if(e.target.id == 'btnDelete' || e.target.id == 'iDelete'){
    ...
    let totalHours = document.getElementById('totalHours');
    totalHours.innerText = parseInt(totalHours.innerText) - parseInt(hours);
    if(totalHours.innerHTML == 0){
      document.getElementById('empty').style.display = "";
    }
  }
  if(e.target.id == 'btnDeleteSubject' || e.target.id == 'iDeleteSubject'){
    document.getElementById(e.target.getAttribute('data-id')).remove();
    let hours = e.target.getAttribute('data-hours')
    let totalHours = document.getElementById('totalHoursSubjects');
    totalHours.innerText = parseInt(totalHours.innerText) - parseInt(hours);
    if(totalHours.innerHTML == 0){
      document.getElementById('emptySubjects').style.display = "";
    }
  }
});
